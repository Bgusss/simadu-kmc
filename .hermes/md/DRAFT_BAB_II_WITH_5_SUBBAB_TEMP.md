...
Ananto, dkk. (2019) mengklasifikasikan kategori pengaduan masyarakat pada kanal LAPOR! menggunakan Artificial Neural Network (ANN) yang dikombinasikan dengan SMOTE untuk mengatasi ketidakseimbangan data dan metode pemilihan kata kunci untuk seleksi fitur, menghasilkan tingkat akurasi dari 428 kata kunci yang berpengaruh. Studi ini terbatas pada satu kanal resmi pemerintah dan mengandalkan model yang harus dilatih ulang dengan data yang sudah dikategorikan. Sistem yang dikembangkan dalam Tugas Akhir ini berbeda karena mengambil aduan dari beberapa kanal media sosial sekaligus (Facebook dan Instagram) serta memanfaatkan model AI yang sudah dilatih sebelumnya (Gemma) sehingga tidak memerlukan pelatihan ulang, sekaligus menangani istilah dialek lokal yang tidak dibahas pada studi tersebut.

Zarly, dkk. (2026) membangun sistem klasifikasi pengaduan otomatis berbasis web menggunakan teknik klasifikasi otomatis di Dinas Kependudukan dan Pencatatan Sipil Kabupaten Pesisir Selatan, dengan integrasi Laravel dan Flask API, dievaluasi menggunakan tingkat akurasi. Sistem tersebut spesifik untuk satu instansi dan satu algoritma metode machine learning lama yang membutuhkan pelatihan model. Sistem yang dikembangkan dalam Tugas Akhir ini mencakup klasifikasi otomatis ke 32 OPD sekaligus tanpa proses pelatihan model karena berbasis LLM generatif.

Prayogo, dkk. (2026) meneliti klasifikasi validitas pengaduan masyarakat (klasifikasi dua kategori valid/tidak valid) menggunakan pendekatan deep learning terhadap 2.000 data aduan, dilatarbelakangi temuan bahwa 13% instansi pemerintah daerah tidak memenuhi SLA 100% pada tahun 2023. Studi ini berhenti pada klasifikasi dua kategori validitas aduan tanpa cara kerja tindak lanjut. Sistem yang dikembangkan dalam Tugas Akhir ini melangkah lebih jauh dengan filter spam dua tahap, klasifikasi banyak kategori (kategori, sub-kategori, OPD), serta cara kerja tiket dengan eskalasi otomatis berbasis SLA 24 jam.
...
Aplikasi web juga memungkinkan akses bersamaan dari berbagai perangkat dan lokasi, sehingga cocok untuk sistem yang melayani banyak pengguna seperti dashboard pengelolaan aduan publik. Dalam konteks KMC, pendekatan berbasis web memungkinkan petugas dari berbagai OPD untuk memantau dan menangani aduan tanpa terikat pada satu perangkat atau lokasi kantor tertentu.

---

#### 2.2.3 Manajemen Aduan

Manajemen aduan adalah proses sistematis untuk menerima, mencatat, mengklasifikasikan, menindaklanjuti, dan menyelesaikan keluhan atau laporan dari masyarakat (ISO 10002:2018). Sistem manajemen aduan yang efektif harus mampu menangani jumlah besar, memberikan transparansi kepada pelapor, serta memastikan tanggung jawab penanganan oleh instansi terkait. Standar ISO 10002:2018 menetapkan pedoman untuk merancang dan menerapkan proses penanganan keluhan yang efektif dan efisien, termasuk prinsip-prinsip seperti kemudahan akses, kecepatan respon, objektivitas, kerahasiaan, dan pendekatan berorientasi pelanggan.
...
Pendekatan ini penting karena tidak semua kelompok masyarakat menggunakan platform yang sama. Generasi muda cenderung aktif di media sosial, sementara kelompok lain mungkin lebih nyaman menggunakan email atau telepon. Dengan mengintegrasikan berbagai saluran ke dalam satu sistem terpusat, KMC dapat memastikan bahwa setiap aduan—dari mana pun asalnya—tercatat, terpantau, dan tertangani dengan standar yang sama.

---

#### 2.2.5 Notifikasi Real-Time

Notifikasi real-time adalah cara kerja penyampaian informasi secara instan kepada pengguna pada saat terjadinya suatu kejadian atau perubahan status dalam sistem (Nugroho & Purnama, 2020). Sistem notifikasi real-time memungkinkan pengguna untuk segera mengetahui informasi penting tanpa harus melakukan pemeriksaan manual secara berkala, sehingga meningkatkan kecepatan respon dalam pengambilan keputusan. Teknologi yang umum digunakan untuk notifikasi real-time meliputi teknologi notifikasi otomatis.
...
Klasifikasi aduan adalah proses pengelompokan aduan ke dalam kategori tertentu berdasarkan isi dan konteks pesan yang disampaikan (Wicaksono & Purwarianti, 2020). Klasifikasi teks secara otomatis memanfaatkan teknik pemrosesan bahasa alami untuk menganalisis dan mengkategorikan dokumen teks ke dalam kelompok yang sudah ditetapkan sebelumnya, sehingga mempercepat proses penanganan dan meningkatkan ketepatan pembagian tugas. Teknik klasifikasi yang umum digunakan meliputi berbagai teknik klasifikasi AI.
...
Metode deteksi duplikasi dapat menggunakan pendekatan kemiripan teks berbasis metode penghitungan kemiripan teks yang membandingkan konten aduan dengan laporan yang masuk dalam rentang waktu tertentu, sehingga aduan duplikat dapat dikelompokkan secara otomatis.
...
Sistem ticketing adalah cara kerja pengelolaan permintaan layanan atau laporan yang menggunakan tiket sebagai unit pencatatan, pelacakan, dan penyelesaian setiap kasus (Mazia dkk., 2021). Setiap tiket menggambarkan satu aduan atau permasalahan yang memiliki siklus hidup mulai dari pembuatan, penugasan, penanganan, hingga penyelesaian. Dengan pendekatan ticketing, setiap aduan tercatat dengan nomor unik sehingga mudah dilacak status penanganannya secara transparan, mendukung cara kerja disposisi ke pihak yang bertanggung jawab, dan menyediakan riwayat penanganan yang berguna untuk evaluasi kinerja di masa mendatang.
...
Natural Language Processing (NLP) adalah bidang ilmu komputer yang fokus pada interaksi antara komputer dan bahasa manusia (Jurafsky & Martin, 2023). NLP memungkinkan komputer untuk memahami teks atau percakapan manusia, memahami maksudnya, dan memberikan respons yang sesuai—mirip seperti cara manusia membaca dan memahami sebuah pesan.

Dalam sistem klasifikasi aduan, NLP berperan untuk menganalisis teks aduan yang ditulis masyarakat dengan gaya bahasa sehari-hari, termasuk istilah lokal atau singkatan informal, kemudian mengambil informasi penting seperti jenis permasalahan, lokasi kejadian, dan tingkat urgensi. Teknologi ini menjadi fondasi bagi model AI untuk memahami konteks aduan tanpa perlu format input yang kaku atau terstruktur.

---

#### 2.2.15 Large Language Model (LLM)

Large Language Model (LLM) adalah model kecerdasan buatan berbasis arsitektur model AI modern yang dilatih pada kumpulan teks yang sangat banyak untuk memahami dan menghasilkan bahasa natural (Vaswani et al., 2017). Berbeda dengan model metode machine learning lama yang perlu dilatih ulang untuk setiap tugas spesifik, LLM memiliki kemampuan menghasilkan yang memungkinkan model untuk menyelesaikan berbagai tugas bahasa hanya dengan pemberian instruksi atau prompt yang tepat—tanpa perlu data latihan tambahan yang banyak.

Dalam sistem pengelolaan aduan, LLM digunakan untuk mengklasifikasikan teks aduan ke dalam kategori, sub-kategori, dan OPD tujuan secara otomatis. Model ini mampu memahami konteks aduan meskipun ditulis dengan bahasa informal atau menggunakan istilah lokal Ketapang, karena telah dilatih pada kumpulan teks yang sangat banyak bahasa yang sangat luas. Pendekatan ini jauh lebih efisien dibanding metode konvensional yang memerlukan ratusan atau ribuan data yang sudah dikategorikan untuk setiap kategori baru.
...
Framework ini juga dilengkapi dengan sistem middleware untuk autentikasi dan autorisasi, migration untuk pengelolaan struktur database secara memiliki versi, serta cara kerja antrian tugas untuk menangani proses berat di background secara tidak sinkron. Ekosistem Laravel yang kaya—termasuk sistem autentikasi API Laravel untuk autentikasi API dan sistem monitoring antrian Laravel untuk monitoring queue—ditambah dengan komunitas pengembang yang besar serta dokumentasi yang lengkap, menjadikan Laravel salah satu framework PHP paling populer untuk pengembangan aplikasi web modern.
...
PHP mendukung cara pemrograman berorientasi objek secara penuh, termasuk konsep-konsep pemrograman objek. Hal ini memungkinkan pengembang untuk menulis kode yang modular, terstruktur, dan mudah dikelola dalam skala proyek yang besar.

---

#### 2.2.18 MySQL

MySQL adalah Sistem Manajemen Basis Data Relasional open-source yang menggunakan Structured Query Language (SQL) untuk mengelola dan memanipulasi data (Lim & Silalahi, 2023). MySQL menjadi pilihan yang tepat karena mampu menangani aspek keamanan seperti pengaturan nama host, hak akses, dan kata sandi. Selain itu, MySQL memiliki performa tinggi dalam menangani operasi baca dan tulis data dalam jumlah besar (Jubilee Enterprise, 2018 dalam Lim & Silalahi, 2023).

MySQL mendukung berbagai fitur penting untuk pengembangan aplikasi modern, seperti transaction dan ACID compliance untuk menjaga integritas data, pengindeksan untuk mempercepat proses pencarian, batasan kunci asing untuk konsistensi relasi antar tabel, prosedur tersimpan dan pemicu untuk logika bisnis di level database, serta replication dan backup untuk keamanan data.
...
Pemrograman Berorientasi Objek (PBO) atau Object-Oriented Programming (OOP) adalah cara pemrograman yang mengorganisasikan perangkat lunak ke dalam objek-objek yang terdiri atas data (atribut) dan perilaku (method) (Rosa & Shalahuddin, 2018). PBO menjadi cara pemrograman yang dominan dalam pengembangan perangkat lunak modern karena mampu menggambarkan objek dunia nyata ke dalam struktur kode yang lebih mudah dipahami dan dikelola.

PBO memiliki empat pilar utama. **Encapsulation (Enkapsulasi)** adalah konsep penyembunyian detail implementasi internal suatu objek dari dunia luar, sehingga objek hanya dapat diakses melalui antarmuka yang telah ditentukan dan melindungi data dari modifikasi yang tidak diinginkan. **Inheritance (Pewarisan)** adalah cara kerja yang memungkinkan suatu class untuk mewarisi atribut dan method dari class lain, sehingga mengurangi duplikasi kode dan mendukung penggunaan ulang komponen. **Polymorphism (Polimorfisme)** adalah kemampuan suatu objek untuk mengambil banyak bentuk, di mana method yang sama dapat memiliki perilaku yang berbeda tergantung pada class yang mengimplementasikannya. **Abstraction (Abstraksi)** adalah proses penyederhanaan kompleksitas dengan menampilkan hanya informasi yang relevan dan menyembunyikan detail yang tidak perlu, dapat diimplementasikan melalui abstract class atau interface.

Pada bahasa PHP dan framework Laravel, konsep-konsep pemrograman objek diterapkan secara luas—setiap model Eloquent merupakan class yang mewarisi class Model dasar, controller merupakan class yang menangani request HTTP, dan middleware merupakan class yang memproses request sebelum mencapai controller.

---

#### 2.2.21 Blade Template Engine

Blade adalah template engine bawaan Laravel yang menyediakan sintaks sederhana namun powerful untuk membuat tampilan antarmuka pengguna (Laravel Documentation, 2023). Blade menggunakan sintaks khusus seperti `{{ }}` untuk menampilkan data dan `@directive` untuk struktur kontrol. Blade mendukung template inheritance sehingga layout dapat digunakan ulang, menyediakan komponen dan slot untuk membuat elemen antarmuka yang reusable, serta melakukan pengamanan otomatis pada output untuk mencegah serangan XSS. Blade juga terintegrasi langsung dengan framework Laravel tanpa konfigurasi tambahan, serta mendukung perintah kondisional seperti `@if`, `@foreach`, `@switch`, dan perintah khusus yang memungkinkan pengembang mendefinisikan sintaks template sesuai kebutuhan proyek.
