...
@section('title', 'Edit OPD - SIMADU KMC')
@section('page-title')
        ...
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-white border-0 py-3 rounded-top-4">
                <h5 class="mb-0 fw-bold text-dark d-flex align-items-center">
                    <i class="fa-solid fa-edit text-primary me-2"></i> Form Edit OPD
                </h5>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('admin.opd.update', $opd->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control rounded-3 @error('name') is-invalid @enderror" value="{{ old('name', $opd->name) }}" placeholder="Masukkan nama OPD" required style="height: 42px; border-color: #cbd5e1;">
                        @error('name')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="username" class="form-control rounded-3 @error('username') is-invalid @enderror" value="{{ old('username', $user->username ?? '') }}" placeholder="Masukkan username login" required style="height: 42px; border-color: #cbd5e1;">
                        @error('username')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" name="email" class="form-control rounded-3 @error('email') is-invalid @enderror" value="{{ old('email', $user->email ?? '') }}" placeholder="Masukkan email aktif" required style="height: 42px; border-color: #cbd5e1;">
                        @error('email')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
