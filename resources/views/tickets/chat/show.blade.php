...
function showMsgInfo(time, sender, deliveredAt, readAt) {
    ...
    if (old) old.remove();
    const html = `<div class="modal fade msg-info-modal" id="msgInfoModal" tabindex="-1"><div class="modal-dialog modal-sm modal-dialog-centered"><div class="modal-content"><div class="modal-header bg-primary text-white py-2"><h6 class="modal-title fw-bold"><i class="fas fa-info-circle me-2"></i>Info Pesan</h6><button type="button" class="btn-close btn-close-white btn-sm" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="mb-2"><span class="text-muted small text-uppercase fw-bold">Pengirim</span><div class="fw-semibold">${sender}</div></div><div><span class="text-muted small text-uppercase fw-bold">Waktu</span><div class="fw-semibold">${time}</div></div></div></div></div></div>`;
...
async function pollAdminChat() {
    ...
    try {
        ...
        data.messages.forEach(function(message) {
            ...
            if (!existing) chatCanvas.insertAdjacentHTML('beforeend', adminMessageMarkup(message));
