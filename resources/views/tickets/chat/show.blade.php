...
<div class="d-flex align-items-center justify-content-between mb-3">
    ...
    <div class="d-flex align-items-center gap-2 d-xl-none">
        ...
        <button type="button" class="btn btn-warning text-dark rounded-circle p-0 d-flex align-items-center justify-content-center shadow-sm" data-bs-toggle="modal" data-bs-target="#mobileUpdateStatusModal" style="width: 36px; height: 36px;" title="Perbarui Status">
...
<div class="row">
        ...
        <div class="chat-card">
            ...
            <header class="chat-head d-flex align-items-center gap-3">
                <div class="chat-avatar p-0 overflow-hidden">
                    @if($opdPhoto)
                        <img src="{{ $opdPhoto }}" alt="{{ $ticket->assignedOpd?->name }}" class="w-100 h-100" style="object-fit: cover; border-radius: 50%;">
                    @else
                                ...
                                <div class="chat-bubble-wrap {{ $mine ? 'mine' : 'theirs' }}">
                                    <article class="chat-message {{ $mine ? 'mine' : 'theirs' }}">
                                        ...
                                        <div class="chat-meta">{{ $message->created_at?->format('H:i') }}
                                            @if($mine)
                                                <span class="chat-receipt {{ $message->read_by_opd ? 'read' : 'sent' }}" title="{{ $message->read_by_opd ? 'Dibaca OPD' : 'Terkirim' }}"></span>
                                            @endif
                                        </div>
                                    </article>
                                    ...
                                    <div class="chat-msg-actions">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-{{ $mine ? 'end' : 'start' }} chat-ctx-menu shadow-lg">
                                            @if($mine)
                                                <li><a class="dropdown-item chat-info-btn" href="#" onclick="showMsgInfoFromEl(this); return false;" data-sent-at="{{ $sentAtStr }}" data-sender="Admin KMC" data-delivered-at="{{ $deliveredAtStr }}" data-read-at="{{ $readAtStr }}"><i class="fas fa-info-circle me-2 text-muted"></i>Info</a></li>
                                            @endif
                                            @php
                                            ...
                                            <li>
                                                <form action="{{ route('tickets.chat.delete', $message) }}" method="POST" onsubmit="deleteMsgAjax(event, this)">
                                                    @csrf @method('DELETE')
                        ...
                        @empty
                            <div class="h-100 d-flex align-items-center justify-content-center text-center text-muted">
                                <div>
                                    ...
                                    <small>Mulai koordinasi dengan OPD melalui kolom pesan di bawah.</small>
                                </div>
                            </div>
                        @endforelse
                    </div>
    ...
    <div class="col-xl-4 d-none d-xl-block mb-4">
        ...
        <aside class="chat-side-card">
            <div class="card-header fw-bold"><i class="fas fa-sync-alt text-warning me-2"></i>Perbarui Status</div>
            <div class="card-body">
                <form action="{{ route('tickets.status', $ticket) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <select name="status" class="form-select mb-3" required>
                        <option value="diterima" @selected($ticket->status==='diterima')>Diterima</option>
                        <option value="proses_disposisi" @selected($ticket->status==='proses_disposisi')>Proses Disposisi</option>
                        <option value="diproses" @selected($ticket->status==='diproses')>Diproses</option>
                        <option value="dijawab" @selected($ticket->status==='dijawab')>Dijawab</option>
                        <option value="selesai" @selected($ticket->status==='selesai')>Selesai</option>
                        <option value="eskalasi" @selected($ticket->status==='eskalasi')>Eskalasi</option>
        ...
        <div class="modal-content border-0 rounded-4 overflow-hidden shadow-lg">
            ...
            <div class="modal-body p-4 bg-white">
                ...
                <div class="row g-2 mb-3">
                    ...
                    <div class="col-6">
                        <div class="ticket-info-tile p-2.5 bg-light rounded-3 border">
                            <div class="ticket-label text-muted small"><i class="fas fa-building me-1"></i>OPD</div>
                            <div class="fw-semibold small text-dark text-truncate">{{ $ticket->assignedOpd?->name ?? '-' }}</div>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="ticket-info-tile p-2.5 bg-light rounded-3 border">
                ...
                @endif
                <a href="{{ route('tickets.show', $ticket) }}" class="btn btn-outline-primary w-100 mt-3 fw-semibold py-2">
        ...
        <div class="modal-content border-0 rounded-4 overflow-hidden shadow-lg">
            <div class="modal-header bg-warning text-dark py-3">
                <h5 class="modal-title fs-6 fw-bold mb-0" id="mobileUpdateStatusModalLabel">
                    <i class="fas fa-edit me-2"></i>Perbarui Status Tiket
                </h5>
            ...
            <div class="modal-body p-4 bg-white">
                <form action="{{ route('tickets.status', $ticket) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <label class="form-label fw-bold small text-secondary mb-1">Pilih Status Tiket</label>
                    <select name="status" class="form-select mb-3" required style="border-radius: 10px;">
                        <option value="diterima" @selected($ticket->status==='diterima')>Diterima</option>
                        <option value="proses_disposisi" @selected($ticket->status==='proses_disposisi')>Proses Disposisi</option>
                        <option value="diproses" @selected($ticket->status==='diproses')>Diproses</option>
                        <option value="dijawab" @selected($ticket->status==='dijawab')>Dijawab</option>
...
document.querySelectorAll('#admin-chat-messages [data-chat-message-id]').forEach(function(el) {
    ...
    if (id) adminKnownMessageIds.add(id);
});

async function pollAdminChat() {
    if (adminPolling || document.hidden) return;
    adminPolling = true;
    const wasNearBottom = adminIsNearBottom();
    try {
        const response = await fetch('{{ route('tickets.chat.poll', $ticket) }}', { headers: { 'Accept': 'application/json' }, credentials: 'same-origin' });
        ...
        data.messages.forEach(function(message) {
            if (!adminKnownMessageIds.has(message.id)) {
                adminKnownMessageIds.add(message.id);
                const existing = chatCanvas.querySelector('[data-chat-message-id="' + message.id + '"]');
                if (!existing) {
                    chatCanvas.insertAdjacentHTML('beforeend', adminMessageMarkup(message));
                    if (!message.mine) {
