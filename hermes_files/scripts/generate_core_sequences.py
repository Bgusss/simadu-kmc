...
def make_sequence_verifikasi_duplikasi():
    ...
    add_message(root, "m8", "8. Mengupdate status & membuat tiket", "A3", "A5", 440)
...
def make_sequence_eskalasi_sla():
    ...
    # Messages
    add_message(root, "m1", "1. Memicu pengecekan berkala", "A1", "A2", 100)
    add_message(root, "m2", "2. Meminta daftar tiket aktif", "A2", "A4", 140)
    add_message(root, "m3", "3. Mengembalikan data tiket", "A4", "A2", 180, True)
    add_message(root, "m4", "4. Memproses eskalasi & SLA", "A2", "A3", 220)
    add_message(root, "m5", "5. Memperbarui tingkat prioritas & SLA baru", "A3", "A3", 260)
    add_message(root, "m6", "6. Menyimpan perubahan & status log", "A3", "A5", 320)
