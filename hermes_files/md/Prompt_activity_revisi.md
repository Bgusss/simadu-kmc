...
**Catatan revisi:** Format diubah dari `stateDiagram-v2` menjadi `flowchart TD`, karena `flowchart` adalah representasi Mermaid yang lebih sesuai dengan notasi UML Activity Diagram (decision node berbentuk diamond, start/end node eksplisit, dan label panah pada percabangan).
...
- Alur:
  ...
  5. Dari "Menyimpan Data", masuk ke Decision: "Validasi Data?".
     - Jika "Tidak": Loop kembali ke pengisian form (masing-masing sesuai jalur Tambah atau Ubah).
